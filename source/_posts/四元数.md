...
但使用第一种表示法，我们可以更容易地展示四元数和复数之间的相似性(以下两点内容为个人大白话理解)。
- 复数可以看作是一个 **实数** 加上一个 **一维向量**，而不是一个二维向量；
- 四元数可以看作是一个**实数** 加上一个 **三维向量**，而不是一个四维向量；
...
& \quad +(s_ax_b + s_bx_a + y_ay_b -y_bz_a)\boldsymbol{i} \\
